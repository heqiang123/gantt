...
<template>
  <div class="checkout-page">
    <div class="c-header">
      <dee-readable-form :form-data="readBealformData" :span-value="5" />
    </div>
    <div class="c-body">
      ...
      <div class="c-step-info">
        ...
        <div class="step-info-body">
          ...
          <div v-for="(item, index) in checkData" :key="index" class="check-item">
            <span class="check-item-0">{{ item.instrument }}</span>
            <span class="check-item-i"><input v-model="item.data" class="value-content" type="text" @blur="bFun(index)"></span>
            <span class="check-item-2">要素：<Unit :icon-value="item.value" :classification-icon="item.classification" /></span>
            <span class="check-item-3"><svg-icon v-if="item.passed" class="passed-icon" icon-class="passed" /></span>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    bFun(index) {
      this.$set(this.checkData[index], 'passed', true)
    },
    uploadFun() {
