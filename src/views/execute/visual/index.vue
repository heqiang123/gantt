    ...
    <div class="c-body">
      ...
      <div class="c-step-info">
        ...
        <div class="step-info-body">
          <!-- <el-row v-for="(item, index) in checkData" :key="index" class="check-item" :gutter="20">
          ...
          <div>
            <div v-for="(item, index) in allData" :key="index" class="log-item">
              <span class="num-content">{{ index + 1 }}</span>
              <span class="d-i-cotent">
                {{ item.na1 }}
              </span>
              <span class="o-b-conten">
                <span class="el-icon-check" @click="deleteItem(index)" />
                <span class="el-icon-close" @click="deleteItem(index)" />
              </span>
            </div>
          </div>
        </div>
  ...
  data() {
    return {
      ...
      allData: [
        ...
        {
          na1: '.部分区域由于熔体压力不够，在该区域截面形成的凹坑'
        },
        {
          na1: ' 由于工艺或模具原因，在边缘分型面处所产生的废边'
        },
        {
          na1: '用指甲刮过划伤处，无段落感'
        },
        {
          na1: ' 因模具、包装或操作等问题造成，分可擦出及不可擦出'
        },
        {
          na1: '因工艺原因内部出现的可见的空气泡'
        },
        {
          na1: '金属零件表面不可有电镀不良现像'
        }
